...
public class Vista extends JFrame {
	...
	private JFormattedTextField fecha_inicioTF, llegada_arteTF,
			pedido_prueba_colorTF, ok_prueba_colorTF;
	private JFormattedTextField llegada_ajustesTF, aprobacion_ocTF, envio_ocTF,
			primer_entregaTF;
	private JFormattedTextField segunda_entregaTF, inicio_instalacionTF,
			fin_instalacionTF, envio_cierreTF;
	private JFormattedTextField fecha_optima_pcTF, fecha_optima_ocTF,
			fecha_optima_entregaTF;
	private JFormattedTextField fecha_optima_inicioTF, fecha_optima_finTF,
			fechaOptimaArteTF;
	private JFormattedTextField arteTF, okRealTF, okIdealTF;

	private JTextField retraso_llegada_arteTF, retraso_pedido_pruebaTF,
			retraso_ok_colorTF, retraso_aprobacion_ocTF,
			retraso_orden_compraTF, retraso_primer_entregaTF,
			retraso_segunda_entregaTF, retraso_inicio_colocacionTF,
			retraso_fin_colocacionTF, retraso_fin_cierreTF;
	private JTextField campañaTF, formatoTF;

	private JLabel retraso_llegada_arteL, retraso_pedido_pruebaL,
			retraso_ok_colorL, retraso_aprobacion_ocL, retraso_orden_compraL,
			retraso_primer_entregaL, retraso_segunda_entregaL,
			retraso_inicio_colocacionL, retraso_fin_cierreL,
			retraso_fin_colocacionL;
	private JLabel campañanaL, formatoL, fecha_inicioL, fechaOptimaArteL;
	private JLabel llegada_ajustesL, aprobacion_ocL, envio_ocL,
			primer_entregaL, pedido_prueba_colorL;
	private JLabel llegada_arteL, ok_prueba_colorL, segunda_entregaL,
			inicio_instalacionL, fin_instalacionL;
	private JLabel envio_cierreL, fecha_optima_pcL, fecha_optima_ocL,
			fecha_optima_entregaL;
	private JLabel fecha_optima_inicioL, fecha_optima_finL;
	private JButton actualizarCampaña, exitB, verRetrasos, expoAexcelB,
			nuevaCB, importarExcelB, resumenB, verOptimosB;
	...
	private VerOptimos vOptimosHandler;
	...
	private class ActualizarButtonHandler implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			System.out.println(estado_viejo);
	...
	public ArrayList<ArrayList<String>> jbotonAarrayDeArrays(
			JComboBox<Campañas> boton) {
		int size = boton.getItemCount();
		boolean a;
		for (int i = 0; i < size; i++) {
			Campañas cam = boton.getItemAt(i);
			System.out.println(campaToArray(cam));
			todasLasCampañas.add(campaToArray(cam));
		}
		return todasLasCampañas;

	}

	public ArrayList<String> campaToArray(Campañas c) {
		ArrayList<String> arrayCampa = new ArrayList<String>();

		arrayCampa.add(c.getNombre_campaña());
		arrayCampa.add(c.getFormatoYcantidad());
		arrayCampa.add(TestUtils.fechaToString(c.getFecha_inicio()));
		arrayCampa.add(TestUtils.fechaToString(c.getFechaArteOptima()));
		arrayCampa.add(TestUtils.fechaToString(c.getLlegada_arte()));
		arrayCampa.add(TestUtils.fechaToString(c.getFechaPcOptima()));
		arrayCampa.add(TestUtils.fechaToString(c.getPedido_prueba_color()));
		arrayCampa.add(TestUtils.fechaToString(c.getOk_prueba_color()));
		arrayCampa.add(TestUtils.fechaToString(c.getLlegada_ajustes()));
		arrayCampa.add(TestUtils.fechaToString(c.getFechaOptimaOC()));
		arrayCampa.add(TestUtils.fechaToString(c.getAprobacion_oc()));
		arrayCampa.add(TestUtils.fechaToString(c.getEnvio_oc()));
		arrayCampa.add(TestUtils.fechaToString(c.getFechaOptimaEntrega()));
		arrayCampa.add(TestUtils.fechaToString(c.getPrimer_entrega()));
		arrayCampa.add(TestUtils.fechaToString(c.getSegunda_entrega()));
		arrayCampa.add(TestUtils.fechaToString(c.getFechaOptimaInicio()));
		arrayCampa.add(TestUtils.fechaToString(c.getInicio_instalacion()));
		arrayCampa.add(TestUtils.fechaToString(c.getFechaOptimaFin()));
		arrayCampa.add(TestUtils.fechaToString(c.getFin_instalacion()));
		arrayCampa.add(TestUtils.fechaToString(c.getEnvio_cierre()));
		try {
			arrayCampa.add(c.retrasoLlegadaArte());
		} catch (NullPointerException e) {
			System.out.println("no hay retraso en al llegada de arte");
		}

		try {
			arrayCampa.add(c.retrasoPedidoPrueba());
		} catch (NullPointerException e) {
			System.out.println("no hay retraso en al pedido prueba");
		}

		try {
			arrayCampa.add(c.retraso_okColor());
		} catch (NullPointerException e) {
			System.out.println("no hay retraso en ok Color");
		}

		try {
			arrayCampa.add(c.retrasoAprobacionOC());
		} catch (NullPointerException e) {
			System.out.println("no hay retraso en la aprobacion OC");
		}
		try {
			arrayCampa.add(c.retrasoOrdenCompra());
		} catch (NullPointerException e) {
			System.out.println("no hay retraso en la orden de compra");
		}
		try {
			arrayCampa.add(c.retrasoPrimerEntrega());
		} catch (NullPointerException e) {
			System.out.println("no hay retraso en la primer entrega");
		}
		try {
			arrayCampa.add(c.retrasoEntregaFinal());
		} catch (NullPointerException e) {
			System.out.println("no hay retraso en la entrega final");
		}
		try {
			arrayCampa.add(c.retrasoInicioColocacion());
		} catch (NullPointerException e) {
			System.out.println("no hay retraso en inicio colocacion");
		}
		try {
			arrayCampa.add(c.retrasoFinColocacion());
		} catch (NullPointerException e) {
			System.out.println("no hay retraso en fin colocacion");
		}
		try {
			arrayCampa.add(c.retrasoEnvioCierre());
		} catch (NullPointerException e) {
			System.out.println("no hay retraso en envio cierre");
		}
		arrayCampa.add(TestUtils.fechaToString(c.arte()));

		arrayCampa.add(TestUtils.fechaToString(c.okReal()));
		arrayCampa.add(TestUtils.fechaToString(c.okIdeal()));
	...
	public class ExpoAexcelbuttonHandler implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			todasLasCampañas = jbotonAarrayDeArrays(allCampañasB);
	...
	public class ImportarExcelbuttonHandler implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			for (ArrayList<String> c : todasLasCampañas) {
				...
				campa = new Campañas(impor_nombre, impor_formato,
						TestUtils.toDateWithTime(impot_fecha_inicio),
						TestUtils.toDateWithTime(impor_llegada_arte),
						TestUtils.toDateWithTime(impor_pedido_prueba_color),
						TestUtils.toDateWithTime(impor_ok_pruebacolo),
						TestUtils.toDateWithTime(impor_llegada_justes),
						TestUtils.toDateWithTime(impor_aprobacion_oc),
						TestUtils.toDateWithTime(impor_envio_oc),
						TestUtils.toDateWithTime(impor_primer_entrega),
						TestUtils.toDateWithTime(impor_segunda_entrega),
						TestUtils.toDateWithTime(impor_inicio_instalacion),
	...
	public ArrayList<String> guardar_estado() {
		ArrayList<String> estado = new ArrayList<String>(Arrays.asList(
				campañaTF.getText(), formatoTF.getText(),
				fecha_inicioTF.getText(), fechaOptimaArteTF.getText(),
				llegada_arteTF.getText(), fecha_optima_pcTF.getText(),
				pedido_prueba_colorTF.getText(), ok_prueba_colorTF.getText(),
				llegada_ajustesTF.getText(), fecha_optima_ocTF.getText(),
				aprobacion_ocTF.getText(), envio_ocTF.getText(),
				...
				retraso_llegada_arteTF.getText(),
				retraso_pedido_pruebaTF.getText(),
				retraso_ok_colorTF.getText(),
				retraso_aprobacion_ocTF.getText(),
				retraso_orden_compraTF.getText(),
				retraso_primer_entregaTF.getText(),
				retraso_segunda_entregaTF.getText(),
				retraso_inicio_colocacionTF.getText(),
				retraso_fin_colocacionTF.getText(),
				retraso_fin_cierreTF.getText(), arteTF.getText(),
				okRealTF.getText(), okIdealTF.getText()));
		return estado;
	...
	public void crearGUI() {
		...
		llegada_ajustesL = new JLabel("Fecha Llegada ajustes: ",
				SwingConstants.LEFT);
		aprobacion_ocL = new JLabel("Fecha Aprobaciones OCs: ",
				SwingConstants.LEFT);
		envio_ocL = new JLabel("Fecha Envio OC: ", SwingConstants.LEFT);
		primer_entregaL = new JLabel("Fecha Primer Entrega: ",
				SwingConstants.LEFT);
		pedido_prueba_colorL = new JLabel("Fecha Pedido Color: ",
				SwingConstants.LEFT);
		llegada_arteL = new JLabel("Fecha Llegada Arte: ", SwingConstants.LEFT);
		ok_prueba_colorL = new JLabel("Fecha Ok Prueba Color: ",
				SwingConstants.LEFT);
		segunda_entregaL = new JLabel("Fecha Segunda Entrega: ",
				SwingConstants.LEFT);
		inicio_instalacionL = new JLabel("Fecha Inicio Instalacion: ",
				SwingConstants.LEFT);
		fin_instalacionL = new JLabel("Fecha Fin Instalacion: ",
				SwingConstants.LEFT);
		envio_cierreL = new JLabel("Fecha Envio Cierre: ", SwingConstants.LEFT);
		fechaOptimaArteL = new JLabel("Fecha Optima Arte: ",
				SwingConstants.LEFT);
		...
		fecha_optima_entregaL = new JLabel("Fecha Optima Entrega: ",
				SwingConstants.LEFT);
		fecha_optima_entregaTF = new JFormattedTextField(format);
		fecha_optima_inicioL = new JLabel("Fecha Inicio PC: ",
				SwingConstants.LEFT);
		...
		retraso_llegada_arteL = new JLabel("Retraso Llegada Arte: ",
				SwingConstants.LEFT);
		retraso_llegada_arteTF = new JFormattedTextField(format);
		retraso_pedido_pruebaL = new JLabel("Retraso Pedido Prueba: ",
				SwingConstants.LEFT);
		retraso_pedido_pruebaTF = new JFormattedTextField(format);
		retraso_ok_colorL = new JLabel("Retraso OK Color: ",
				SwingConstants.LEFT);
		retraso_ok_colorTF = new JFormattedTextField(format);
		retraso_aprobacion_ocL = new JLabel("Retraso Aprobacion OC: ",
				SwingConstants.LEFT);
		retraso_aprobacion_ocTF = new JFormattedTextField(format);
		retraso_orden_compraL = new JLabel("Retraso Orden Compra: ",
				SwingConstants.LEFT);
		retraso_orden_compraTF = new JFormattedTextField(format);
		retraso_primer_entregaL = new JLabel("Retraso Primer Entrega: ",
				SwingConstants.LEFT);
		retraso_primer_entregaTF = new JFormattedTextField(format);
		retraso_segunda_entregaL = new JLabel("Retraso Entrega Final: ",
				SwingConstants.LEFT);
		retraso_segunda_entregaTF = new JFormattedTextField(format);
		retraso_inicio_colocacionL = new JLabel("Retraso Inicio Colocacion: ",
				SwingConstants.LEFT);
		retraso_inicio_colocacionTF = new JFormattedTextField(format);
		retraso_fin_colocacionL = new JLabel("Retraso Fin Colocacion: ",
				SwingConstants.LEFT);
		retraso_fin_colocacionTF = new JTextField();
		retraso_fin_cierreL = new JLabel("Retraso Fin Cierre: ",
				SwingConstants.LEFT);
		...
		verOptimosB = new JButton("Ver Optimos");
		vOptimosHandler = new VerOptimos();
		verOptimosB.addActionListener(vOptimosHandler);
		...
		campañaTF.setDocument(new FixedSizeDocument(25));
		formatoTF.setDocument(new FixedSizeDocument(30));
	...
	public class VerRetrasos implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			verRetrasos.setVisible(false);
			verOptimosB.setVisible(true);
		}

	}

	public class VerOptimos implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			// escondo los optimos
			...
			verRetrasos.setVisible(true);
			verOptimosB.setVisible(false);
	...
	public void calcularFechas() {
		try {
			String strDate = new SimpleDateFormat("dd/MM/yyyy")
					.format(fecha_inicioTF.getValue());
		...
		try {
			String strDate2 = new SimpleDateFormat("dd/MM/yyyy")
					.format(llegada_arteTF.getValue());
		...
		try {
			String strDate3 = new SimpleDateFormat("dd/MM/yyyy")
					.format(pedido_prueba_colorTF.getValue());
		...
		try {
			String strDate4 = new SimpleDateFormat("dd/MM/yyyy")
					.format(ok_prueba_colorTF.getValue());
		...
		try {
			String strDate5 = new SimpleDateFormat("dd/MM/yyyy")
					.format(llegada_ajustesTF.getValue());
		...
		try {
			String strDate6 = new SimpleDateFormat("dd/MM/yyyy")
					.format(envio_ocTF.getValue());
		...
		try {
			String strDate7 = new SimpleDateFormat("dd/MM/yyyy")
					.format(primer_entregaTF.getValue());
		...
		try {
			String strDate8 = new SimpleDateFormat("dd/MM/yyyy")
					.format(segunda_entregaTF.getValue());
		...
		try {
			String strDate9 = new SimpleDateFormat("dd/MM/yyyy")
					.format(inicio_instalacionTF.getValue());
		...
		try {
			String strDate10 = new SimpleDateFormat("dd/MM/yyyy")
					.format(fin_instalacionTF.getValue());
			campa.setFin_instalacion(TestUtils.toDateWithTime(strDate10));
		} catch (IllegalArgumentException a) {
			campa.setFin_instalacion(TestUtils.toDateWithTime(""));
		}
		try {
			String strDate11 = new SimpleDateFormat("dd/MM/yyyy")
					.format(envio_cierreTF.getValue());
		...
		try {
			String strDate11 = new SimpleDateFormat("dd/MM/yyyy")
					.format(aprobacion_ocTF.getValue());
		...
		try {
			fechaOptimaArteTF.setText(TestUtils.fechaToString(campa
					.getFechaArteOptima()));
		...
		try {
			fecha_optima_pcTF.setText(TestUtils.fechaToString(campa
					.getFechaPcOptima()));
		} catch (NullPointerException a) {
		...
		try {
			fecha_optima_ocTF.setText(TestUtils.fechaToString(campa
					.getFechaOptimaOC()));
		} catch (NullPointerException a) {
		...
		try {
			fecha_optima_entregaTF.setText(TestUtils.fechaToString(campa
					.getFechaOptimaEntrega()));
		} catch (NullPointerException a) {
			fecha_optima_entregaTF.setText("");
		}

		try {
			fecha_optima_inicioTF.setText(TestUtils.fechaToString(campa
					.getFechaOptimaInicio()));
		} catch (NullPointerException a) {
			fecha_optima_inicioTF.setText("");
		}

		try {
			fecha_optima_finTF.setText(TestUtils.fechaToString(campa
					.getFechaOptimaFin()));
		} catch (NullPointerException a) {
		...
		try {
			retraso_inicio_colocacionTF
					.setText(campa.retrasoInicioColocacion());
	...
	public void dameCampa(Campañas c) {
		...
		pedido_prueba_colorTF.setText(TestUtils.fechaToString(c
				.getPedido_prueba_color()));
		ok_prueba_colorTF.setText(TestUtils.fechaToString(c
				.getOk_prueba_color()));
		llegada_ajustesTF.setText(TestUtils.fechaToString(c
				.getLlegada_ajustes()));
		...
		primer_entregaTF
				.setText(TestUtils.fechaToString(c.getPrimer_entrega()));
		segunda_entregaTF.setText(TestUtils.fechaToString(c
				.getSegunda_entrega()));
		inicio_instalacionTF.setText(TestUtils.fechaToString(c
				.getInicio_instalacion()));
		fin_instalacionTF.setText(TestUtils.fechaToString(c
				.getFin_instalacion()));
		envio_cierreTF.setText(TestUtils.fechaToString(c.getEnvio_cierre()));
		retraso_ok_colorTF.setText(c.retraso_okColor());
		retraso_pedido_pruebaTF.setText(c.retrasoPedidoPrueba());
		retraso_aprobacion_ocTF.setText(c.retrasoAprobacionOC());
		retraso_orden_compraTF.setText(c.retrasoOrdenCompra());
		try {
			retraso_primer_entregaTF.setText(c.retrasoPrimerEntrega());
		} catch (NullPointerException e) {
		}

		try {
			retraso_segunda_entregaTF.setText(c.retrasoEntregaFinal());
		} catch (NullPointerException e) {
		}

		try {
			retraso_inicio_colocacionTF.setText(c.retrasoInicioColocacion());
		} catch (NullPointerException e) {
		}

		try {
			retraso_llegada_arteTF.setText(c.retrasoLlegadaArte());
		} catch (NullPointerException e) {
		}

		try {
			retraso_fin_cierreTF.setText(c.retrasoEnvioCierre());
		} catch (NullPointerException e) {
		}

		try {
			retraso_fin_colocacionTF.setText(c.retrasoFinColocacion());
		} catch (NullPointerException e) {
		}

		try {
			fecha_optima_pcTF.setText(TestUtils.fechaToString(c
					.getFechaPcOptima()));
		} catch (NullPointerException e) {
		}

		try {
			fecha_optima_ocTF.setText(TestUtils.fechaToString(c
					.getFechaOptimaOC()));
		} catch (NullPointerException e) {
		}

		try {
			fechaOptimaArteTF.setText(TestUtils.fechaToString(c
					.getFechaArteOptima()));
		} catch (NullPointerException e) {
		}

		try {
			fecha_optima_finTF.setText(TestUtils.fechaToString(c
					.getFechaOptimaFin()));
		} catch (NullPointerException e) {
		}

		try {
			fecha_optima_inicioTF.setText(TestUtils.fechaToString(c
					.getFechaOptimaInicio()));
		} catch (NullPointerException e) {
		}

		try {
			fecha_optima_entregaTF.setText(TestUtils.fechaToString(c
					.getFechaOptimaEntrega()));
		} catch (NullPointerException e) {
		}
